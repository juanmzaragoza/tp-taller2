...
class BeFriendController(flask_restful.Resource):
	...
	def post(self):
		try:
			 user_sender_id, user_rcv_id, msg = self._get_friend_request_data(request)
			 self._validate_user_id(user_sender_id)
			 self._validate_user_id(user_rcv_id)
			 be_friend_request = self._create_be_friend_request(user_sender_id, user_rcv_id, msg)
	...
	def _get_friend_request_data(self, request):
		...
		return body['senderUserId'], body['rcvUserId'], body['message']
