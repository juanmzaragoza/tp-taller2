...
class TestCommentsApi(unittest.TestCase):
    ...
    def test_successful_get_stories_comment_should_return_status_200(self, auth_mock):
        ...
        for comment in response_data:
            print(comment)
        self.assertTrue(all(comment["story_type"] == "normal" for comment in response_data))
